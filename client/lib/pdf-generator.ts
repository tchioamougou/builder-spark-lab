...
export const generateAdmissionPDF = (data: StudentData): void => {
  ...
  doc.text(`Niveau d'enseignement général : ${data.niveauEnseignement}`, 20, yPos);
  yPos += 7;
  ...
  // Pied de page
  doc.setFontSize(8);
  doc.text('Web site: www.efpps-edu.org  E-mail: info@efpps-edu.org  P.O. Box: 100  Tel: +237699475876', 105, 270, { align: 'center' });
  doc.setFont('helvetica', 'bold');
  doc.text(`Autorisation N° Décision N° 2523/D/MINSANTE/SG/DRH of the 2nd of August 2021`, 105, 280, { align: 'center' });
...
export const generateConfirmationPDF = (data: StudentData): void => {
  ...
  doc.setFontSize(12);
  doc.setFont('helvetica', 'normal');
  doc.text('École Privée de Formation des Professionnels de la Santé de Meiganga', 105, 45, { align: 'center' });
  ...
  doc.text('Nous accusons réception de votre demande d\'admission pour la formation :', 20, yPos);
  yPos += 10;
  ...
  doc.text('Votre dossier est actuellement en cours de traitement. Vous recevrez une notification', 20, yPos);
  yPos += 8;
  doc.text('concernant la suite de la procédure dans les plus brefs délais.', 20, yPos);
  yPos += 20;
