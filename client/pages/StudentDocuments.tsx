...
import { generateDocumentsList } from "@/lib/pdf-utils";
import {
  FileText,
  Download,
  Upload,
...
export default function StudentDocuments() {
  ...
  const getStatusColor = (statut: string) => {
    switch (statut) {
      ...
      case "Soumis": case "En cours": return "bg-blue-100 text-blue-800";
  ...
  const getStatusIcon = (statut: string) => {
    switch (statut) {
      case "Disponible": case "Validé": case "Prêt": return <CheckCircle className="h-4 w-4" />;
      case "En cours": case "Soumis": return <Clock className="h-4 w-4" />;
      ...
      <div className="space-y-6">
              ...
              <DialogContent className="sm:max-w-[425px]">
                ...
                <div className="grid gap-4 py-4">
                  ...
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Textarea id="motif" className="col-span-3" rows={3} placeholder="Expliquez pourquoi vous avez besoin de ce document..." />
                  </div>
                  <div className="grid grid-cols-4 items-center gap-4">
                    <Label htmlFor="urgent" className="text-right">Urgence</Label>
                    <Select>
                      <SelectTrigger className="col-span-3">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {documentRequests.filter(req => req.statut === "En cours").length}
              </div>
              <p className="text-xs text-muted-foreground">
                En traitement
              </p>
        ...
        <Tabs defaultValue="documents" className="space-y-6">
          ...
          <TabsContent value="documents" className="space-y-4">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">Rechercher dans mes documents</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle>Documents personnels ({filteredDocuments.length})</CardTitle>
                <CardDescription>
                  ...
                  <TableBody>
                    {filteredDocuments.map((doc) => (
                      <TableRow key={doc.id}>
                        ...
                        <TableCell>{new Date(doc.dateCreation).toLocaleDateString('fr-FR')}</TableCell>
                        <TableCell>{doc.taille}</TableCell>
                        <TableCell>
                          <div className="flex items-center space-x-2">
                            {getStatusIcon(doc.statut)}
                            <Badge variant="secondary" className={getStatusColor(doc.statut)}>
                              {doc.statut}
                            </Badge>
                          </div>
                        </TableCell>
